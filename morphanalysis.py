"""
SMorph: a Novel Tool for Automated Morphometric Analysis.
Copyright (C) 2021  Swanand Lab
...
def _validate_image_params(contrast_ptiles, threshold_method):
...
def try_all_threshold(img):
    skimage.filters.try_all_threshold(img, (10, 6), False)
    plt.show()


def best_poynomial_degree(cell_sholl):
    best_degree, best_r2 = 2, 0
    x, y = (cell_sholl.non_zero_distances_from_soma.reshape(-1, 1),
            cell_sholl.non_zero_no_of_intersections)
    for i in range(2, 9):
        x_ = preprocessing.PolynomialFeatures(
        degree=i, include_bias=False).fit_transform(x)
        # create a linear regression model
        polynomial_model = linear_model.LinearRegression().fit(x_, y)
        r2 = polynomial_model.score(x_, y)
        if best_r2 < r2:
            best_r2, best_degree = r2, i
    return best_degree
...
class Cell:
    ...
    def __init__(
        ...
        contrast_ptiles: tuple of size 2 containing bridging percentile values for pixel intensities.
        threshold_method: otsu, isodata, li, mean, minimum, triangle, or yen

        """

        contrast_ptiles, threshold_method = _validate_image_params(contrast_ptiles, threshold_method)
        ...
        self.threshold_image = self.threshold(contrast_ptiles, threshold_method)
    ...
    def threshold(self, contrast_ptiles, method):
        ...
        # Contrast stretching
        p_low, p_hi = np.percentile(self.gray_cell_image, contrast_ptiles)
        img_rescale = skimage.exposure.rescale_intensity(self.gray_cell_image, in_range=(p_low, p_hi))

        THRESHOLD_METHODS = {'otsu': threshold_otsu, 'isodata': threshold_isodata,
                             'li': threshold_li, 'yen': threshold_yen, 'mean': threshold_mean,
                             'minimum': threshold_minimum, 'triangle': threshold_triangle }
...
class Skeleton:
    ...
    def __init__(self, cell_image, image_type, contrast_ptiles=(0, 100), threshold_method='otsu'):
        """
        Args:
...
class Sholl:
    ...
    def sholl_results(self, plot=False):
        ...
        if plot==True:
            ...
            ax.imshow(self.padded_skeleton)
            ...
            # plot circles on skeleton
            for r,_ in concentric_coordinates.items():
    ...
    def polynomial_fit(self, plot=False):
        ...
        self.non_zero_no_of_intersections = self.no_of_intersections[:last_intersection_index+1]
        self.non_zero_distances_from_soma = self.distances_from_soma[:last_intersection_index+1]
...
class analyze_cells:
    ...
    def __init__(
        ...
        self.features = self.get_features(dataset, contrast_ptiles, threshold_method)
    ...
    def get_features(self, dataset, contrast_ptiles, threshold_method):
        dataset_features=[]
        self.targets=[]
        ...
        try:
            os.remove(SKIPPED_CELLS_FILENAME)
        except OSError as e:
        ...
        for group_no, group in enumerate(dataset):
            ...
            for cell_no, cell_image in enumerate(group):
                ...
                print('Analyzing:', self.file_names[cell_count])
                ...
                try:
                    ...
                    skeleton = Skeleton(cell_image, self.image_type,
                                        contrast_ptiles=contrast_ptiles,
                                        threshold_method=threshold_method)
                    sholl = Sholl(cell_image, self.image_type, self.polynomial_degree, self.shell_step_size,
                                contrast_ptiles=contrast_ptiles,
                                threshold_method=threshold_method)
                    # plt.imsave('cache/' + self.file_names[cell_count-1][13:], astrocyte.cleaned_image_filled_holes)
                    ...
                    bad_cells_idx.append(cell_count - 1)
                    print('Warning: Skipping analysis of',
                        f'"{self.file_names[cell_count - 1]}" due to {err}.')
                    with open(SKIPPED_CELLS_FILENAME, 'a') as skip_file:
                        skip_file.write(self.file_names[cell_count - 1] + '\n')

            self.group_counts.append(group_cell_count)

        file_names = [cell_name for idx, cell_name in enumerate(self.file_names)
    ...
    def plot_feature_scatter_matrix(self, on_features):
        ...
        feature_data = pd.DataFrame(self.features, columns=_ALL_FEATURE_NAMES)
        subset_features = feature_data[on_features]

        scaler = preprocessing.MinMaxScaler()
        scaler.fit(subset_features)
        X = pd.DataFrame(scaler.transform(subset_features), columns=on_features)

        axis = pd.plotting.scatter_matrix(X, figsize=(18, 18))
    ...
    def pca(self, n_PC, on_features, color_dict, marker):
        ...
        feature_data = pd.DataFrame(np.array(self.features), columns=_ALL_FEATURE_NAMES)
        subset_features = feature_data[on_features].to_numpy()
        ...
        return pca_object.components_, pca_object.get_covariance()
    ...
    def plot_feature_significance_heatmap(self):
        ...
        sorted_feature_names = np.array(self.pca_features)[
